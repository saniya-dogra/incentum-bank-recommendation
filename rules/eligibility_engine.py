...
from rules.co_applicant_rules import process_co_applicants
...
def bom_engine(data):
    ...
    # -------------------------------------------------
    ok, msg, combined_income, youngest_age = process_co_applicants(data)
    if not ok:
        return False, msg

    # Replace income with combined income
    data["monthly_income"] = combined_income
    ...
    checks = [
        ...
        check_tenure(youngest_age, data["tenure_years"]),
        check_property(
            data["property_age"],
            data["city_type"],
            data["loan_category"],
